function [params_fit, info] = vkf_optim(y, nRestarts, varargin)
    ...
    if opts.verbose
        ...
        info.bic = -2 * ll_fit + 3 * log(length(y));
        info.aic = -2 * ll_fit + 2 * 3;
        info.ll = ll_fit;
        % Approximate log model evidence using Laplace approximation
        % LME ≈ log(likelihood) - 0.5 * n_params * log(n_trials)
        info.lme = ll_fit - 0.5 * 3 * log(length(y));
        info.accuracy = mean((p_pred > 0.5) == y);
