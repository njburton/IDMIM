...
for subj = 1subjCount
    ...
    for condNum = 1size(partData(subj).parcel, 2)
    
        try
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % eHGF fit
            eHGFFits{subj, condNum} = tapas_fitModel(partData(subj).cart(, condNum), partData(subj).parcel(, condNum), ...
                                            'tapas_ehgf_config', ...
            ...
            h = sgtitle(append('Subject ', num2str(subj, '%03d'), '  Condition ', ...
                      partData(subj).subjCondition{}, '  RMSE ', num2str(eHGFFits{subj, condNum}.optim.RMSE, '%.2f'), '  Block ', partData(subj).blockType{condNum}));
            ...
            h = sgtitle(append('Subject ', num2str(subj, '%03d'), '  Condition ', ...
                      partData(subj).subjCondition{}, '  RMSE ', num2str(eHGFNoVolatilityFits{subj, condNum}.optim.RMSE, '%.2f'), '  Block ', partData(subj).blockType{condNum}));
            set(h, 'fontweight', 'bold');
            ...
            pause(2);
            h = sgtitle(append('Subject ', num2str(subj, '%03d'), '  Condition ', ...
                      partData(subj).subjCondition{}, '  RMSE ', num2str(eHGFNoPercpUncertFits{subj, condNum}.optim.RMSE, '%.2f'), '  Block ', partData(subj).blockType{condNum}));
            set(h, 'fontweight', 'bold');
