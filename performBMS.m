...
if optionsFile.doExcludeData==1
    [exclArray,optionsFile] = excludeData(optionsFile,cohortNo,subCohort,'excludeData',optionsFile.cohort(cohortNo).exclMode.ModelLevel);
end
            ...
            for iMouse = 1:nSize
                ...
                if ~any(exclArray(iExcl).ID==iMouse)
                    for iModel = 1:numel(optionsFile.model.space)
                        ...
                        res.prc_param(iInclMouse,iModel).ptrans = est.p_prc.ptrans(optionsFile.modelSpace(iModel).prc_idx);
                        res.obs_param(iInclMouse,iModel).ptrans = est.p_obs.ptrans(optionsFile.modelSpace(iModel).obs_idx);
                    end
