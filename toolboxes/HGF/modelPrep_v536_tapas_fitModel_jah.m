function r = modelPrep_v536_tapas_fitModel_jah(responses, inputs, varargin)

%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Additional FEATURES %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
% - input prc/obs/opt config files as struct
% - Option to choose between MAP and ML estimation --> r.ML_est 
%       (currently not implemented: starting point 0 for all free param)
% - saving LME decomposed into different contributions (accord. to Lapl.
%       approx.) --> LME_decomposed
% - saving LogLl separately for combined response model (bin & cont)
% - try...catch optimization (re-sample if obj fct cannot be evaluated)
% - option to set seed for random initializations (multistart procedure)
%%%
% - save iterations of opt algo
% - handle NaN values in trialLogLls -> set logLl to -Inf
%%%
...
% Default Model inversion technique
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
r.ML_est = 0; % 0=MAP estimation, 1=MLE
...
if nargin > 6 && ~isempty(varargin{5})
    r.c_opt.seed_nRandInit = varargin{5};
else
    r.c_opt.seed_nRandInit = NaN;
end
...
optres = optimrun(r, nlj, init, opt_idx, opt_algo, r.c_opt);
...
r.optim.trialLogLlsplit = optres.trialLogLlsplit; % split trial LogLl
...
r.optim.LME_decomposed = optres.LME_decomposed; %decomposed LME
%%%
r.optim.iter  = optres.iter; % iterations of opt algo
%%%
...
if isfield(r.c_opt, 'nRandInit') && r.c_opt.nRandInit > 0
    for i = 1:r.c_opt.nRandInit
        ...
        % set seed for rng
        if isnan(r.c_opt.seed_nRandInit)
            rng('shuffle');
        else
            rng(r.c_opt.seed_nRandInit);
        end
        ...
        stable = 0; % initialize variable indicating stable state
        while stable == 0
            try
        [dummy1, dummy2, rval, err] = nlj(init);
        if rval ~= 0
...
trialLogLls(r.irr) = [];
logLl = sum(trialLogLls); % work with "not r.irr" (???)
if isnan(logLl)
    negLogLl = Inf;
else
    negLogLl = -logLl;
...
% --------------------------------------------------------------------------------------------------
function optres = optimrun(r, nlj, init, opt_idx, opt_algo, c_opt)
...
obj_fun = @(p_opt) restrictfun(r, nlj, init, opt_idx, p_opt);
...
if any(isinf(H(:))) || any(isnan(H(:))) || any(eig(H)<=0)
    if isfield(optres, 'T')
        ...
        % decomposed LME
        LME_decomposed.logjoint = -optres.valMin;
        LME_decomposed.postpredcorr = 1/2*log(1/det(H));
        LME_decomposed.const = d/2*log(2*pi);
    else
...
optres.Sigma = Sigma;
optres.Corr = Corr;
optres.trialLogLlsplit = trialLogLlsplit; % split trial LogLl
optres.negLl = negLl;
optres.negLj = negLj;
optres.LME = LME;
optres.LME_decomposed = LME_decomposed; % decomposed LME
...
% --------------------------------------------------------------------------------------------------
function val = restrictfun(r, f, arg, free_idx, free_arg)
...
% Evaluate
if r.ML_est == 0 %MAP
    val = f(arg); %nlj as obj fct
elseif r.ML_est == 1 %ML
    [dummy, val] = f(arg); %nLl as obj fct
end
