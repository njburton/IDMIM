% run_kf_vkf_separate_plots.m
% Run and plot VKF and KF on real SPIRL binary data (separate plots)
...
%% --- Load your real SPIRL data ---
u = agent(1).task.data.u;         % 301×1 binary input
y = agent(1).task.data.y;    % 301×1 binary outcome
nt = length(y);

% Check binary validity
assert(all(ismember(y, [0 1])), 'Outcome y must be binary (0 or 1)');
assert(length(u) == nt, 'Input and output must have same number of trials');
